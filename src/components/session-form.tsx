...
export default function SessionForm({ playerId, playerName }: SessionFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Date */}
      <div>
        <label htmlFor="session-date" className="mb-2 block text-sm font-medium text-foreground">
          Date
        </label>
        <input
          id="session-date"
      ...
      {/* Duration */}
      <div>
        <label htmlFor="session-duration" className="mb-2 block text-sm font-medium text-foreground">
          Duration (minutes)
        </label>
        <input
          id="session-duration"
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="e.g. tactical drills, match minutes, gym focus..."
          className="w-full rounded-lg border border-card-border bg-white px-3 py-2 text-sm text-foreground placeholder:text-gray-400 focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent"
        />
      </div>
      ...
      {rpe !== null && duration.length > 0 && parseInt(duration, 10) > 0 && (
        <div className="rounded-lg bg-gray-50 px-4 py-3 text-center">
          ...
          <p className="mt-1 text-2xl font-bold text-foreground">
            {rpe * parseInt(duration, 10)} <span className="text-sm font-normal text-muted">AU</span>
          </p>
        </div>
      )}
