...
import type { Player, PlayerRiskSnapshot, WellnessEntry } from "@/lib/types";
...
interface PlayerWithData {
  ...
  riskSnapshot: PlayerRiskSnapshot | null;
}

interface PlayersListProps {
  playersData: PlayerWithData[];
}

export default function PlayersList({ playersData }: PlayersListProps) {
  const [search, setSearch] = useState("");

  const filtered = playersData.filter((d) =>
    d.player.name.toLowerCase().includes(search.toLowerCase()) ||
    d.player.position.toLowerCase().includes(search.toLowerCase())
  );

  return (
    <>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-muted">{playersData.length} players in squad</p>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {filtered.map(({ player, latestWellness, riskSnapshot }) => (
          <Link
            key={player.id}
          ...
          >
            ...
            <div className="mt-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {riskSnapshot && <RiskLevelBadge level={riskSnapshot.riskLevel} />}
              </div>
              {latestWellness ? (
                <div className="flex items-center gap-1.5">
                  <span className="text-xs text-muted">Wellness</span>
                  <WellnessBadge score={latestWellness.overallScore} />
                </div>
              ) : (
